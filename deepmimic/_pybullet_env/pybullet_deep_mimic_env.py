...
from _pybullet_env.humanoid_stable_pd import HumanoidStablePD
import time
...
class PyBulletDeepMimicEnv(Env):
  ...
  def __init__(self, arg_parser=None, enable_draw=False, pybullet_client=None,
    ...
    self._num_agents = 1
    # print("Pybullet client is None?", pybullet_client is None) #True
    ...
    self._reset_count = 0 #(Myi) how much reset() is called?
    print("Initialization strategy: {:s}".format(init_strategy))
    self.reset()

  def reset(self):
    self._reset_count +=1
    # print("================="*11, "reset is called", self._reset_count,"times!")

    if not self._isInitialized:
      #============================== 한번만 불리는 애들? ==========
      # print("================="*11, "reset(internal setup) is called")
      if self.enable_draw:
      ...
      motionPath = _pybullet_data.getDataPath() + "/" + motion_file[0]
      #motionPath = pybullet_data.getDataPath()+"/motions/humanoid3d_backflip.txt"
      self._mocapData.Load(motionPath)
      timeStep = self.timeStep
      useFixedBase = False
      self._humanoid = HumanoidStablePD(self._pybullet_client, self._mocapData,
  ...
  def update(self, timeStep):
    ...
    for i in range(1):
      ...
      if self.desiredPose:
        kinPose = self._humanoid.computePose(self._humanoid._frameFraction)
        # print("[update] kinPose : ", kinPose, "its length: ", len(kinPose))
        # [update] kinPose :  [0.2589361575725036, 0.8725985386966046, 0.24268376448770532, 
        # 0.01878255046904087, 0.1745646893978119, -0.0453055240213871, 0.9834234714508057, \
        # 0.01683179847896099, 0.0012379634426906705, -0.03576100245118141, 0.9992178082466125, 
        # 0.025757748633623123, 0.024006927385926247, -0.12648655474185944, 0.9913433194160461, 
        # -0.24333885312080383, -0.09502466768026352, 0.03229288011789322, 0.9647350311279297, 
        # -0.8662962278326546, 0.0731767788529396, 0.015535325743258, -0.03669170290231705, 0.9965227246284485, 
        # -0.7720018029212952, -0.22860828042030334, 0.40073728561401367, 0.43721985816955566, 0.7780672787096107, 
        # 0.15443341434001923, 0.1435096561908722, 0.13057051599025726, 0.9687654972076416, -0.43133664040816383, 
        # -0.14322637021541595, -0.052289120852947235, 0.09700394421815872, 0.983535647392273, 0.18542756140232086, 
        # -0.06228572502732277, -0.25853386521339417, 0.9459900259971619, 0.41609882910734336] its length:  43
        ...
        if self._useStablePD:
          # print("At update: self._useStablePD : ", self._useStablePD) #True 계속떠서 지움,,
          ...
          if usePythonStablePD:
            ...
            #right now, take this block
            # print("At update: ", "computeAndApplyPDForces directly..") 계속떠서 지움,,
            self._humanoid.computeAndApplyPDForces(self.desiredPose,
                                                maxForces=maxForces)
        else:
          print("set Joint Motors in update??") #nope
