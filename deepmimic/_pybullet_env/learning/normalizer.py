...
class Normalizer(object):
  ...
  def update(self):
    ...
    # # print("before reduce ", self.new_sum)
    # new_sum = MPIUtil.reduce_sum(self.new_sum)
    # # print("after reduce ", new_sum)
    # # print("num process ? " , MPIUtil.get_num_procs()) # (Myi) 1 이라서 b비포애프터 같은.. chatgpt말대로 mpi 무시하고 일단짜자
  ...
  def set_mean_std(self, mean, std):
    size = self.get_size()
    # print("[Inside normalizer.py ] size == ", size)
    # print(self.mean, "is mean with its size", self.mean.size)
    is_array = isinstance(mean, np.ndarray) and isinstance(std, np.ndarray)
    # print("[Inside normalizer.py ] is_array == ", is_array)
    if not is_array:
      assert (size == 1)
      mean = np.array([mean])
      std = np.array([std])
