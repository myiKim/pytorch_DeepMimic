...
class ReplayBuffer(object):
  ...
  def store(self, path): #이게 불려야 샘플버퍼의 카운트가 올라감.
    ...
    if (n > 0):
      ...
      if path.check_vals():
        ...
        # print("call _add_sample_buffers here")
        self._add_sample_buffers(idx)
        # print("call _add_sample_buffers finished here")
  ...
  def _add_sample_buffers(self, idx): #이게 불려야 샘플버퍼의 add()가 불려서 궁극적으로 카운트가 올라가는듯.
    flags = self.buffers['flags']
    # print(flags, 1&np.array([1,1,0,0])) #그럼, flags는 [0,0,0,0] 비트연산은 [1,1,0,0] 나옴
    
    for key in self._sample_buffers:
      # print("[_add_sample_buffers] key: ", key, "idx ==", idx)
      # [_add_sample_buffers] key:  1 idx == [374, 375, 376, 377, 378, 379, 380, 381, 382, 383, 
      ...
      # print("filter_idx: ", filter_idx) #이게 엠티 되어버리는게 문제.. 필터에서 걸리는데, (self.buffers[key]가 key==flag일때 0으로 이닛되지만 1로 바뀌지 않는듯..)
      #1로 바뀌는게, 
      # for i in idx:
      #   print("Why 1? , ",self._check_flags(key, flags[i]))
      #   print("Why 2? , ",self.is_path_end(i))
  ...
  def _store_path(self, path, idx):
    ...
    # print("33"*191, self.TERMINATE_KEY, self.PATH_START_KEY, self.PATH_END_KEY)
...
class SampleBuffer(object):
  ...
  def add(self, idx):
    # print("Sample Buffer add called? idx == ", idx)
    for i in idx:
