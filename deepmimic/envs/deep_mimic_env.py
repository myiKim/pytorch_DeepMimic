...
from _pybullet_utils import bullet_client as bc
from pkg_resources import parse_version
from _pybullet_env.pybullet_deep_mimic_env import PyBulletDeepMimicEnv, InitializationStrategy
...
class hmyifEnv(gym.Env):
  """Base Gym environment Myi first env tried to mimic"""
  ...
  def __init__(self, renders=False, arg_file='', test_mode=False,
    ...
    self._p = None
    time_step  = 1 / (240 * 128)#이렇게 안하면 debugdisplay 후루룩 지나가버림.
    self._time_step = time_step
    self._internal_env = None
    renders = True # debugdisplay 화면을 바로 보여줌.
  ...
  def _configure(self, display=None):
    print("============= [deep_mimic_env] _configure ================== ")
    self.display = display

  def seed(self, seed=None):
    print("============= [deep_mimic_env] seed ================== ")
    self.np_random, seed = seeding.np_random(seed)
    return [seed]

  def step(self, action):
    print("============= [deep_mimic_env] step ================== ")
  ...
  def reset(self):
    #새 에피가 시작되는 (디버그디스플레이: 휴머노이드 넘어졌다 다시 일어나는)
    print("============= [deep_mimic_env] reset ================== ")
    # use the initialization strategy
    if self._internal_env is None:
      print("internal env is none..")
      if self.test_mode:
...
class HumanoidMyiFirstBackflipBulletEnv(hmyifEnv):
  metadata = {'render.modes': ['human', 'rgb_array'], 'video.frames_per_second': 50}

  def __init__(self, renders=False):
    # start the bullet physics server
    HumanoidMyiFirstEnv.__init__(self, renders, arg_file="run_humanoid3d_backflip_args.txt")


class HumanoidMyiFirstWalkBulletEnv(hmyifEnv):
  metadata = {'render.modes': ['human', 'rgb_array'], 'video.frames_per_second': 50}

  def __init__(self, renders=False):
    # start the bullet physics server
    HumanoidMyiFirstEnv.__init__(self, renders, arg_file="run_humanoid3d_walk_args.txt")

class CartPoleMyiContinuousBulletEnv5(hmyifEnv):
  metadata = {'render.modes': ['human', 'rgb_array'], 'video.frames_per_second': 50}

  def __init__(self, renders=False):
    # start the bullet physics server
    HumanoidMyiFirstEnv.__init__(self, renders, arg_file="")